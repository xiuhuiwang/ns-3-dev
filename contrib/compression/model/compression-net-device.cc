...
CompressionNetDevice::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::CompressionNetDevice")
    ...
    .AddAttribute ("EnableCompression",
                   "Whether this device will compress packets",
                   BooleanValue (false),
                   MakeBooleanAccessor (&CompressionNetDevice::SetEnableCompression,
                                        &CompressionNetDevice::GetEnableCompression),
                   MakeBooleanChecker())
    .AddAttribute ("EnableDecompression",
                    "Whether this device will decompress packets",
                    BooleanValue(false),
                    MakeBooleanAccessor(&CompressionNetDevice::SetEnableDecompression,
                                        &CompressionNetDevice::GetEnableDecompression),
                     MakeBooleanChecker())
  ;
  return tid;
}
...
CompressionNetDevice::CompressionNetDevice () 
  :
    ...
    m_enableDecompression(false),
    m_tcpHeaderByteCount(0),
    m_seqNumber(0),
    m_tcpPacket(true)
{
...
CompressionNetDevice::TransmitStart (Ptr<Packet> p)
{
  ...
  Time txTime = Seconds (m_bps.CalculateBytesTxTime(p->GetSize ()).GetSeconds());
...
CompressionNetDevice::TransmitComplete (void)
{
  ...
  if (p == 0)
    {
      // std::cout<<"No Packet in the queue!!!! Returning!!!!!"<<std::endl;
      //
...
void
CompressionNetDevice::SetQueue (Ptr<Queue<Packet>> q)
{
...
CompressionNetDevice::Receive (Ptr<Packet> packet)
{
  ...
  if (m_receiveErrorModel && m_receiveErrorModel->IsCorrupt (packet) ) 
    {
      std::cout<<"Corrupt packet in CompressionNetDevice::Receive !!!!"<<std::endl;
      // 
  ...
  else 
    {
      ...
      //
        ...
        //
        //
        // std::cout<<"In Receive of CompressionNetDevice::Receive!!!!"<<std::endl;

//        std::cout<<"m_enableDecompression:"<<m_enableDecompression<<std::endl;
...
//  std::cout<<"Existing Receive!!!!!!!"<<std::endl;
}
...
{
  NS_LOG_FUNCTION_NOARGS ();
  // std::cout<<"protocolNumber:"<<protocolNumber<<std::endl;
  NS_LOG_LOGIC ("p=" << packet << ", dest=" << &dest);
  NS_LOG_LOGIC ("UID is " << packet->GetUid ());

    // std::cout<<"In Send of CompressionNetDevice::Send!!!!"<<std::endl;
...
//    std::cout<<"m_enableCompression:"<<m_enableCompression<<std::endl;
...
void
CompressionNetDevice::SetEnableDecompression(bool enable)
{
    m_enableDecompression = enable;
}

bool
CompressionNetDevice::GetEnableDecompression(void) const
{
    return m_enableDecompression;
}
