...
{
 //   std::cout<<"CompressionHeader::SetData  !!!!!"<<std::endl;
  NS_LOG_FUNCTION_NOARGS ();
  m_size = size;
  m_data = (uint8_t*) realloc(m_data, size);
  memcpy(m_data, data, size);
 // std::cout<<"Exiting CompressionHeader::SetData  !!!!!111111"<<std::endl;
}

uint32_t
CompressionHeader::GetData (uint8_t* data, uint32_t size) const
{
  //  std::cout<<"CompressionHeader::GetData !!!"<<std::endl;
    NS_LOG_FUNCTION_NOARGS ();
  if (size < m_size) {
      std::cout<<"CompressionHeader::GetData - buffer too small!!!"<<std::endl;
    NS_LOG_UNCOND ("CompressionHeader::GetData - buffer too small");
    return -1;
  }
  memcpy(data, m_data, m_size);
 // std::cout<<"Exiting CompressionHeader::GetData !!!!!111111"<<std::endl;
  return m_size;
}
...
{
 //   std::cout<<"CompressionHeader::Serialize !!!"<<std::endl;
  NS_LOG_FUNCTION_NOARGS ();
  Buffer::Iterator it = start;
  it.Write (m_data, m_size);
  //  std::cout<<"Exiting CompressionHeader::Serialize !!!!!111111"<<std::endl;
}

uint32_t
CompressionHeader::Deserialize (Buffer::Iterator start)
{
  //  std::cout<<"CompressionHeader::Deserialize !!!"<<std::endl;
  NS_LOG_FUNCTION_NOARGS ();
  Buffer::Iterator it = start;
  m_data = (uint8_t*) realloc(m_data, m_size);
  it.Read (m_data, m_size);
 //   std::cout<<"Exiting CompressionHeader::Deserialize !!!!!111111"<<std::endl;
